...
from recursive.agent.agents import regular  # Import to register agents
import random
    ...
    def load(self, folder):
        ...
        if os.path.exists(root_node_file):
            with open(root_node_file, "rb") as f:
                ...
                self.memory.root_node = self.root_node
        
        self.memory.load(folder)
...
def story_writing(input_filename,
    ...
    config = {
        ...
        "REASONING": {
            ...
            "update": {},
            "final_aggregate": {
                "prompt_version": "StoryReasonerAggregateEn" if language == "en" else "StoryReasonerAggregateZh",
                "mode": "llm",
                "parse_arg_dict": {
                    "result": ["result"],
                },
    ...
    for item in items:
        ...
        log_id = logger.add("{}/engine.log".format(folder))
...
def book_writing(input_filename,
    ...
    data = read_jsonl(input_filename, False)
    if start is None:
        items = data
    else:
        items = data[start:end]
    ...
    for item in items:
        ...
        folder = "{}/{}".format(root_folder, qstr)
        os.makedirs(folder, exist_ok=True)
        bf = open("{}/book.md".format(folder), "w")
