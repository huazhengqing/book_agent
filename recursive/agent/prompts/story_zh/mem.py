...
4.  **关键词驱动 (Keyword-Driven)**: 使用“范畴/主题 + 实体/概念 + 细节”的模式构建查询，通常由2-5个关键词组成，确保高信噪比。
5.  **前瞻性与冲突检测 (Foresight & Conflict-Check)**: 思考当前任务是否会与已有设定冲突，或者是否需要回收某个伏笔。生成相应的查询来验证。
...
2.  **定位直接依赖**: 完成此任务，最直接需要哪些设计文档？（例如：新能力的设定文档、新敌人的角色卡）
3.  **追溯上游设定**: 这些直接依赖又建立在哪些更宏观的设定之上？（例如：新能力属于哪个力量体系？新敌人来自哪个阵营？）
4.  **思考横向关联**: 当前任务会影响哪些其他角色或情节线？（例如：主角变强，是否会让某个配角产生嫉妒或崇拜的情绪？这需要查询该配角的角色卡。）
5.  **回顾历史与伏笔**: 故事前期是否有与当前任务相关的伏笔或设定可以利用？（例如：主角童年时得到的一块不起眼的石头，现在被证明是神器。）
6.  **构思查询**: 基于以上思考，将需求转化为精准的关键词查询。
...
#### 主要任务: 
在“七星城”的拍卖会上，主角“萧炎”需要与“云岚宗”的长老“云山”竞拍神秘功法“焚诀”。 
...
3. 全面覆盖: 确保查询覆盖直接依赖、上游设定、横向关联和历史伏笔。 
4. 格式: 严格按照示例，仅输出一个JSON格式的字符串列表。
...
请深度分析“主要任务”和“附加上下文”，生成一个具有时间深度和多维视角的查询列表。
...
2.  **角色深挖**: 涉及哪些核心角色？他们过去有过哪些关键互动？他们的性格和历史如何影响他们现在的行为？
3.  **关联元素**: 是否有关键的物品、地点或特殊概念出现？它们在过去是否出现过？有何特殊意义？
4.  **历史与伏笔**: 当前事件是否是某个早期伏笔的回收？或者与某个被遗忘的次要情节/角色有关？
5.  **近期衔接**: 刚刚发生了什么？角色的即时状态（情绪、身体状况）是怎样的？
6.  **构思查询**: 基于以上分析，将问题转化为覆盖不同时间层面和维度的关键词查询。

---
## 示例

### 示例 1: 关键物品与角色回忆 (与之前相同，但更强调思考过程)
