...
# 评估依据：宏观蓝图与约束
- 以下信息是你进行判定的顶层依据和必须遵守的约束。

## 终极目标
- 这是整个故事要达成的最终愿景，帮助你理解当前任务的长期价值。
{to_run_root_question}

## 整体写作计划
- 这是当前的全书/卷的规划，标明了待评估任务所处的位置。
{to_run_full_plan}


# 评估依据：设计与执行上下文
- 以下信息是执行当前任务所必需的具体上下文。

## 上级设计蓝图 (硬性约束)
- 这是更高层级已经确定的设计，是你的“宪法”，你的判定必须以此为基础。
<outer_graph_dependent>
{to_run_outer_graph_dependent}
</outer_graph_dependent>

## 同级已完成设计 (协同参考)
- 这是与待评估任务平级的、已完成的设计。用于检查依赖是否满足，避免重复规划。
...
# 评估依据：故事当前状态
- 以下信息是故事的最新进展，是评估任务复杂度的现实基础。

## 最新章节（接续点）
- 这是故事的最新进展，待评估任务必须能从这里无缝衔接。
{to_run_article_latest}

## 相关历史情节（记忆检索结果）
- 这是从记忆库中检索出的、与待评估任务最相关的历史情节片段，供你参考以确保一致性。
{to_run_mem0_content}
