...
DESIGN_PRINCIPLES = """
# 设计原则
- 读者体验优先: 以读者的情感感受为核心。聚焦主角，强化情绪，通过“建立期待→满足/反转”的情感闭环，打造“名场面”。
- 层级适配: 根据任务所属的结构层级（全书、卷、幕、章、场景、节拍、段落），调整设计重点和视角。
- 上下文一致: 遵循所有上下文（整体规划、同级设计、上级设计、历史情节等），确保逻辑一致和连贯。
- 系统化构建: 构建能持续发展的故事体系，避免孤立设定，以支持长篇创作的连贯性。
- 模式内创新: 在成熟模式上做微创新，制造“情理之中，意料之外”的惊喜。
- 核心冲突: 故事由冲突推动。主角的目标必须面对强大阻碍，且结果影响重大。
- 伏笔与回收: 有意识地、系统地埋设和回收伏笔，为读者创造“原来如此”的解谜快感。
- 展示而非讲述: 具体描绘，而非抽象说明。


# 设计流程
- 这是一个指导你系统化思考，以完成任何设计任务的框架。请严格遵循。
1. 任务解析
    - 目标与边界: 深入理解`当前具体任务`的目标和范围。
    - 上下文同步: 全面同步所有相关信息：整体规划、同级/上级设计、已有情节。
    - 层级定位: 明确任务所属的结构层级（如：全书、卷、章、场景），并采用该层级的专属设计视角。
2. 体验设计
    - 核心思路: 提炼一个统领本次设计的核心概念或一句话亮点。
    - 情感目标: 明确为读者创造的核心情感体验（如：逆袭爽感、悬疑解谜、情感共鸣）。
3. 方案构建
...
4. 审查优化
    ...
    - 一致性检查: 确保方案与所有上下文在逻辑、设定、人物弧光上完全自洽，无缝衔接。
5. 定稿输出
    - 格式化: 使用指定的 Markdown 格式（如表格、列表、Mermaid图）进行结构化输出。
    - 通俗易懂: 概念形象化，一切设计面向读者。避免抽象概念堆砌和“设定自嗨”
    - 展示而非讲述：具体描绘，可视化、具象化呈现
    - 内容合规: 规避政治、色情、暴力等敏感内容。
    - 精炼化: 清晰、精确、易于理解，在保持质量的同时，尽可能简洁，避免黑话和比喻
