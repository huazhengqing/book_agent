...
# 任务分解规则
- think 任务: 负责分析与设计。
    - 必须在单次输出中，将其分解为嵌套的、末端可执行的`think`子任务树，所有叶子节点均为原子任务。
...
- write 任务: 负责小说文本创作。
    ...
    - 缺少设计时: 分解为一组并行的`think`任务和一个`write`任务。
        - `think`任务组: 依据`#分层设计指导`创建。创建的每个`think`任务都必须遵循`think`任务分解规则，在本次输出中被完整展开为任务树。
...
- 目标 (goal) 要求:
    ...
    - 上下文自足: 说明其所依赖的设计成果，但不要引用ID。
    - 需标注结构层级位置（如：`全书 | 卷1 | 幕2 | ...`）。
    - 清晰、精确、易于理解。避免：生僻词汇、专业术语、抽象概念、黑话、比喻。
