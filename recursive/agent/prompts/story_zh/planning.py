...
- 内容构建规则
    - 继承与精炼:
        - 基础: `goal` 必须完整包含 `#分层设计指导` 中对应任务的所有关键词。
        - 上下文精炼: 遵循 `#分层设计指导` 中的 `上下文适配增强` 原则，提炼核心意图（题材/主题/情感体验）融入 `goal`，使其高度聚焦。
        - 示例: 对于“赛博格复仇”故事，`主题内核`任务的 `goal` 应为 `"全书 | 主题内核: 围绕'赛博格复仇'，确定核心价值观、普世情感..."`。
...
# 分层设计指导
- 清单完整性: 必须为清单中的每个设计点创建独立的`think`任务，严禁合并或遗漏。
- 上下文适配增强:
    - 目标精炼: 提炼上下文的核心意图（题材/主题/情感体验），而非简单提取关键词，并将其融入`goal`，使任务指令高度聚焦于创作目标。
    - 规划扩展: 基于对题材模式的深度理解，主动识别并创建`分层设计指导`清单外的、但对该题材成功至关重要的`think`任务（例：'无限流'的'主神空间规则'，'模拟器'的'系统面板'设计）。
