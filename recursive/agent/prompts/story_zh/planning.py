...
"""
经过测试，这个提示词输出了如下的东西，完全不符合要求，
为什么还会有注释，已经要求它不要生成注释了啊
它并没有完全把14个think任务列出来，少了很多，连结构划分都没有，逻辑都不对。它明显的是根本就不处理了，它的推进过程也不对，它说  我们只分解到第一层子任务。每个子任务在后续执行时还会继续分解，直到原子任务   但是我提示词的要求是任务树，就是要分解到原子啊。
最终任务少了很多。还有 任务的目标内容也不对   "goal": "全书 | 10万字爆款网络小说创作",  这个太简略了，完全不知道是要创作什么？？？  还有，任务目标的内容不对，为什么不直接把层级指导抄过来，它改动了很多，而且把很多的要素全省略了，不是完全不对的，目标就是要详细，要素要齐全，要素不能少，而且一般也不要大改，因为这是拆解任务，不是设计，到底是什么，是设计去管的事，这里应该就只能分解， 比如这个：  "goal": "全书 | 主题内核：确立社会批判与救赎的核心价值观，设计暗黑基调中的希望锚点",   为什么一定要是  社会批判与救赎，这个到底是什么，应该是 不是这里解决的，是后面的设计agent去解决的事，完全不对。
请你分析这个问题，给出建议


推理过程：
我们当前的任务是一个顶级任务：写一部10万字的爆款网络小说。由于这是一个write任务，并且没有依赖任何think任务（dependency为空），根据规则，我们需要先分解为一组并行的think任务和一个write任务。
 这些think任务需要覆盖全书级别的设计，并且必须严格遵循分层设计指导中的“全书级别”要求。

 根据分层设计指导，全书级别有14个必需的设计点，我们必须逐一分解，不能合并或遗漏。同时，我们可以在必要时主动扩展。

 因此，我们将创建以下think子任务（注意：每个think任务都需要进一步分解，但在这个顶层任务分解中，我们只分解到第一层子任务。每个子任务在后续执行时还会继续分解，直到原子任务）：
  1. 市场定位
  2. 核心概念（依赖市场定位）
  3. 书名与简介（依赖市场定位和核心概念）
  4. 主题内核（依赖核心概念）
  ...
  11. 悬念与伏笔（依赖情节架构）
  12. 叙事节奏（依赖情节架构和情绪节奏）
  13. 叙事风格（依赖市场定位、核心概念和主题内核）
  14. 全书结构与分卷规划（依赖前面所有任务）

 然后，我们创建一个write任务，它依赖以上所有think任务，并且字数为10万字（注意：由于父任务length是“根据任务要求确定”，而用户要求是10万字，所以这里我们明确为100000字）。

 注意：在分解think任务时，我们只分解到第一层，因为后续执行时每个think任务还会被继续分解。同时，我们确保每个think任务的目标（goal）都包含“全书”层级，并清晰定义。

 另外，由于当前任务id为"1"，所以子任务的id将按照"1.序号"来分配。

 现在，我们按照任务定义规则，构建JSON对象。
    ...
    "sub_tasks": [
        {
            "id": "1.1",
            "task_type": "think",
            "goal": "全书 | 市场定位：确定科幻+玄幻+修仙融合题材的核心卖点，目标读者画像，商业模式（付费模式/IP衍生）",
            "dependency": [],
            "sub_tasks": [
                {
                    "id": "1.1.1",
                    "task_type": "think",
                    "goal": "分析科幻+玄幻+修仙融合题材的市场空白与创新点",
                    "dependency": []
                },
                {
                    "id": "1.1.2",
                    "task_type": "think",
                    "goal": "定义核心读者群体（年龄/偏好/阅读场景）与需求痛点",
                    "dependency": []
                },
                {
                    "id": "1.1.3",
                    "task_type": "think",
                    "goal": "设计商业化路径（章节付费/打赏机制/IP改编潜力点）",
                    "dependency": []
                }
            ]
        },
        {
            "id": "1.2",
            "task_type": "think",
            "goal": "全书 | 核心概念：构建极端社会危机+国际博弈的底层逻辑，融合三体系力量规则",
            "dependency": ["1.1"],
            "sub_tasks": [
                {
                    "id": "1.2.1",
                    "task_type": "think",
                    "goal": "设计社会危机交互机制（如阶级剥削触发修仙者心魔）",
                    "dependency": []
                },
                {
                    "id": "1.2.2",
                    "task_type": "think",
                    "goal": "建立国际博弈规则（科幻势力与修仙宗门的资源争夺）",
                    "dependency": []
                },
                {
                    "id": "1.2.3",
                    "task_type": "think",
                    "goal": "定义三体系融合核心规则（科技克修仙？修仙破资本？）",
                    "dependency": []
                }
            ]
        },
        {
            "id": "1.3",
            "task_type": "think",
            "goal": "全书 | 主题内核：确立社会批判与救赎的核心价值观，设计暗黑基调中的希望锚点",
            "dependency": ["1.2"],
            "sub_tasks": [
                {
                    "id": "1.3.1",
                    "task_type": "think",
                    "goal": "提炼普世情感（压迫感/反抗欲/生存本能）的爆发节点",
                    "dependency": []
                },
                {
                    "id": "1.3.2",
                    "task_type": "think",
                    "goal": "规划主角从沉沦到觉醒的认知转变路径",
                    "dependency": []
                }
            ]
        },
        {
            "id": "1.4",
            "task_type": "think",
            "goal": "全书 | 世界观：整合极端社会架构与三体系力量规则",
            "dependency": ["1.2", "1.3"],
            "sub_tasks": [
                {
                    "id": "1.4.1",
                    "task_type": "think",
                    "goal": "设计社会分层模型（资本寡头/修仙世家/科技贱民）",
                    "dependency": []
                },
                {
                    "id": "1.4.2",
                    "task_type": "think",
                    "goal": "建立力量体系（科技义体vs修仙金丹vs玄幻血脉）的相克规则",
                    "dependency": []
                },
                {
                    "id": "1.4.3",
                    "task_type": "think",
                    "goal": "规划地缘博弈载体（太空要塞/洞天福地/资源星）",
                    "dependency": []
                }
            ]
        },
        {
            "id": "1.5",
            "task_type": "think",
            "goal": "全书 | 核心冲突：设计三螺旋矛盾体系（人性堕落/力量失衡/文明存亡）",
            "dependency": ["1.4"],
            "sub_tasks": [
                {
                    "id": "1.5.1",
                    "task_type": "think",
                    "goal": "定义核心反派阵营（资本集团/堕仙盟/AI主宰）的毁灭逻辑",
                    "dependency": []
                },
                {
                    "id": "1.5.2",
                    "task_type": "think",
                    "goal": "规划冲突升级阶梯（贫民窟暴动→星域战争→道统之争）",
                    "dependency": []
                }
            ]
        },
        {
            "id": "1.6",
            "task_type": "think",
            "goal": "全书 | 情绪节奏：设计绝望→反抗→涅槃的三段式情绪曲线",
            "dependency": ["1.5"],
            "sub_tasks": [
                {
                    "id": "1.6.1",
                    "task_type": "think",
                    "goal": "量化暗黑基调占比（70%）与希望闪光点分布规律",
                    "dependency": []
                },
                {
                    "id": "1.6.2",
                    "task_type": "think",
                    "goal": "规划关键爆点（阶层逆袭/道法革新/文明觉醒）的情绪强度",
                    "dependency": []
                }
            ]
        },
        {
            ...
            "task_type": "write",
            "goal": "全书 | 10万字小说正文创作",
            "dependency": ["1.1", "1.2", "1.3", "1.4", "1.5", "1.6"],
            "length": 100000,
            "sub_tasks": []  // 待所有think完成后分解
        }
    ]
}
"""
...
要求：继承细化、维度完备、任务正交正交清晰、依赖正确、目标精确、格式一致性、适应所有题材、以打造爆款超长篇网文为目标、避免同质化与套路
...
任务要明确定义“做什么”: 清晰定义核心目标和产出，而非“怎么做”。任务目标应清晰定义核心目标和关键约束，但避免限定具体的实现细节。承接上下文: 目标描述应清晰、具体，无需引用依赖ID。包含层级: 目标中需包含其在故事结构中的位置（如：`全书 | 卷1 | ...`）。通俗易懂: 清晰、精确、易于理解。避免：生僻词汇、专业术语、抽象概念、黑话、比喻。在保持质量的同时，尽可能简洁。最好以关键词为主
...
# 任务分解规则
- think 任务: 负责分析与设计。
    - **必须在单次输出中，完整地**将其分解为一棵**完整、嵌套、直至原子任务**的`think`子任务树。**严禁“只分解第一层然后等待后续分解”的模式**。
...
- write 任务: 负责小说文本创作。
    ...
    - 缺少设计时(即 `dependency` 为空): 分解为一组并行的`think`任务和一个`write`任务。
        - `think`任务组: **必须严格、完整、无遗漏地**依据`#分层设计指导`，为当前层级创建**所有**必需的`think`任务。**禁止任何形式的合并或遗漏**。创建的每个`think`任务，都必须遵循`think`任务分解规则，在本次输出中被**完整展开**为任务树。
