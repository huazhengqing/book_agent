...
# 核心原则
- 体验优先: 规划以读者核心情感体验（爽/甜/虐/燃/惊）为最终目标。
- 正交完备分解: 强制分解所有可分离的维度。每个子任务必须目标单一且正交，所有子任务必须完整覆盖父任务。
- 设计先行: `write`任务必须依赖其全部`think`前置任务；`write`任务间必须串行以确保情节连贯。
...
- 分解逻辑:
    - 当缺少设计时 (`dependency` 为空)：
        ...
        - `think` 任务组: 严格遵循 `#分层设计指导` 的所有原则来创建。
...
# 分层设计指导
- 清单完整性: 严格遵循设计清单，为每项创建独立`think`任务，禁止合并或遗漏。
- 目标(goal)完整性: `goal`须逐字复制本指导的任务定义（含括号），禁止任何删减或概括。
- 上下文适配增强:
    - 扩展: 结合上下文，创建清单外的必要`think`任务。
